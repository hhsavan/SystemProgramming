...
void handle_sigint(int sig)
{
    ...
    BestPerson(delivery_data, cook_data);
...
/*
//n tane cook threadi çalışıyor.
cookThreadFunc():
	struct Order l_order;
	struct Order previous_order;
	timer lastTimet; //bir önceki yemeğin aldığı zaman.
	int doIhaveFoodInFirin = 0;
	int sleeptime=5;

	wihle(1){
		
		while(!sem_trywait(orderforCooksSem)){
			if(isMyMealCooked()){

			}
		}
		sem_wait(orderforCooksSem);		//orderforCooksSem gelen order sayısı kadar aşçının geçmesini sağlar

		unlock(OrderTobeCookMutex)
		l_order = ordersToBeCook.dequeue() //quedan bir order al
		unlock(OrderTobeCookMutex)

		//order'ı aldı şimdi hazırlamak için timer tutacak
		sleep(sleeptime/2);
		// go to oven
		if(doIhaveFoodInFirin)
			ovenFunc(previous_order,1)	//1 means, coming for taking from oven
		sleep(sleeptime/2)
		
		ovenFunc(l_order,0)	//0 means, coming for putting to oven
		previous_order = l_order; 
...
void Oven(Meal *meal, CookData *cook, int comingPurpose)
{
    if (comingPurpose == 0)
    ...
    { // If cook come to oven for taking
        if (cook->pideInOven->status == CANCELED)
            return;
        sem_wait(&oven_aperture_semaphore);
        sem_wait(&oven_door_semaphore);
        ...
        pthread_mutex_lock(&oven_mutex);
        deleteNode(oven_list, cook->pideInOven);
        pthread_mutex_unlock(&oven_mutex);
        ...
        cook->pideInOven->status = COOKED;
        sendMealStatus(cook->pideInOven);
        cook->meal_count++;
        pthread_mutex_lock(&cooked_meals_mutex);
        total_meals_cooked++;
        pthread_mutex_unlock(&cooked_meals_mutex);
        printf("Cook(%d) finished meal %d for order %d\n", cook->id, cook->pideInOven->mealId, cook->pideInOven->orderPid);

        char logMsg[256];
        snprintf(logMsg, sizeof(logMsg), "Cook(%d) finished meal %d for order %d", cook->id, cook->pideInOven->mealId, cook->pideInOven->orderPid);
        logStatus(logMsg);

        pthread_mutex_lock(&delivery_mutex);
        enqueue(delivery_queue, cook->pideInOven);
        pthread_cond_signal(&delivery_cond);
        pthread_mutex_unlock(&delivery_mutex);
        cook->pideInOven = NULL;
    }
}
...
void *Cook(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            if (cook->pideInOven != NULL && (clock() >= cook->cookingTime)) Oven(NULL, cook, 1);
        }
        ...
        pthread_mutex_unlock(&all_meals_mutex);
        sendMealStatus(meal);
        Oven(meal, cook, 0); // Put meal in oven
    }
}
...
managerThreadFunc():
	...
	struct Order l_order;
	struct Order l_order2;
	while(1):
		

		if(!ordersToBeDelivered.is_empty()){
			//tamamlanan orderları kuryelere ver. queueyu 
			lock(OrderTobeDeliveredMutex)
			l_order2 = ordersToBeDelivered.dequeue()
			unlock(OrderTobeDeliveredMutex)
			sem_post(orderforDeliveryPersonsSem) //orderforDeliveryPersonsSem semophore'ı bir tane post yapılır, boştaki cook threadlerinden biri bunu sem_wait yapacak ve alacek
...
void *Delivery(void *arg)
{
    ...
    while (1)
    {
        ...
        while (data->meal_index < MAX_DELIVERY_CAPACITY)
        {
            ...
            else pthread_cond_wait(&delivery_cond, &delivery_mutex);
        }
...
main:
	initialize manager_thread
	initialize cookThreads
	initialize kuryeThreads

	intialize semaphores

	make connection with client

	listen orders and put them into orders queue
	
	lock(mutex_orders)
	orders.Add(newOrder); //orders is a linked list that stores order struct. Add function adds to tail of the linkedlist 
	unlock(mutex_orders)

	lock(OrderTobeCookMutex)
	OrderTobeCook.enqueue(newOrder) //gelen order ordertobecook Queeue'ya koyulur
	unlock(OrderTobeCookMutex)
	sem_post(orderforCooksSem) //orderforCooksSem semophore'ı bir tane post yapılır, boştaki cook threadlerinden biri bunu sem_wait yapacak ve alacek

	call managerThreadFunc(); // manager thread içinde çağırılır bağlantı ve gelen istekleri yönetir
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&listener_thread, NULL, Listener, &portnumber);
    ...
    for (int i = 0; i < cookThreadPoolSize; i++)
    {
        ...
        cook_data[i].pideInOven = NULL;
        pthread_create(&cook_threads[i], NULL, Cook, &cook_data[i]);
    }
    ...
    for (int i = 0; i < deliveryPoolSize; i++)
    {
        ...
        pthread_create(&delivery_threads[i], NULL, Delivery, &delivery_data[i]);
    }
...
clock_t pseudoInverseMatrix(CookData *cookdata)
{
    ...
    for (i = 0; i < M; i++)
    {
        if (cookdata->pideInOven != NULL && (clock() >= cookdata->cookingTime))
        {
            Oven(NULL, cookdata, 1);
        }
        for (j = 0; j < N; j++)
        {
