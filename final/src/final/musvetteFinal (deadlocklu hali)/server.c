...
void* phonethreadfunc(void* arg) {
    ...
    while (1) {
        ...
        enqueue(orders_queue, new_order);
        printf("Order received and added to queue\n");
...
void* cookThreadFunc(void* arg) {
    while (1) {
        ...
        pthread_mutex_lock(&orders_mutex);
        Order *order = (Order *)dequeue(orders_queue);
        pthread_mutex_unlock(&orders_mutex);
        ...
        printf("Cook preparing meal for order %d\n", order->id);
        ...
        printf("Cook putting meal for order %d in oven\n", order->id);
        insertAtEnd(oven_list, order);
        sleep(1); // Simulate time
        ...
        printf("Meal for order %d is cooking\n", order->id);
        sleep(1); // Simulate time

        pthread_mutex_lock(&oven_mutex);
        // Simulate taking meal out of oven
        printf("Cook taking meal for order %d out of oven\n", order->id);
        deleteNode(oven_list, order);
        pthread_mutex_unlock(&oven_mutex);

        // Notify manager
        printf("Cook finished meal for order %d\n", order->id);
        pthread_cond_signal(&delivery_cond);

        free(order->meals); // Free the memory allocated for the meals
        free(order); // Free the memory allocated for the order
    }
}
...
void* managerThreadFunc(void* arg) {
    while (1) {
        pthread_mutex_lock(&orders_mutex);
        while (isQueueEmpty(orders_queue)) {
...
void* deliveryThreadFunc(void* arg) {
    while (1) {
        ...
        printf("Delivery person delivering meal for order %d\n", order->id);
        sleep(1); // Simulate time
